                ...
                quote! {
                    {
                        ...
                        let parsed = if let Some(segment) = segment.as_deref() {
                            <#ty as freya_router::routable::FromRouteSegment>::from_route_segment(segment).map_err(|err| #error_enum_name::#error_enum_variant(#inner_parse_enum::#error_name(err)))
                        } else {
                ...
                quote! {
                    {
                        let parsed = {
                            ...
                            <#ty as freya_router::routable::FromRouteSegments>::from_route_segments(&new_segments).map_err(|err| #error_enum_name::#error_enum_variant(#inner_parse_enum::#error_name(err)))
                        };
                        match parsed {
                            Ok(#name) => {
                                #parse_children
...
pub fn parse_route_segments<'a>(
    ...
    if first != Some("") {
        return Err(syn::Error::new(
            route_span,
            format!("Routes should start with /. Error found in the route '{route}'",),
        ));
    }
...
pub(crate) fn create_error_type(
    ...
    for (i, segment) in segments.iter().enumerate() {
        ...
        match segment {
            RouteSegment::Static(index) => {
                let comment = format!(
                    " An error that can occur when trying to parse the static segment '/{index}'.",
                );
            ...
            RouteSegment::Dynamic(ident, ty) => {
                ...
                let comment = format!(
                    " An error that can occur when trying to parse the dynamic segment '/:{ident}'.",
                );
                error_variants.push(quote! {
                    #[doc = #comment]
                    #error_name(<#ty as freya_router::routable::FromRouteSegment>::Err)
                });
            ...
            RouteSegment::CatchAll(ident, ty) => {
                let comment = format!(
                    " An error that can occur when trying to parse the catch-all segment '/:..{ident}'.",
                );
                error_variants.push(quote! {
                    #[doc = #comment]
                    #error_name(<#ty as freya_router::routable::FromRouteSegments>::Err)
                });
    ...
    let comment =
        format!(" An error that can occur when trying to parse the route variant `{route}`.",);
